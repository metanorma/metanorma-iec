...
RSpec.describe Relaton::Render::Iso do
  ...
  it "renders software" do
    ...
    INPUT
    output = <<~OUTPUT
      <formattedref><smallcap>Ribose Inc.</smallcap>. <em>metanorma-standoc</em>. Version 1.3.1. 2019. Available at: <span class='biburl'><fmt-link target='https://github.com/metanorma/metanorma-standoc'>https://github.com/metanorma/metanorma-standoc</fmt-link></span>. [viewed: September 6, 2023].</formattedref>
    OUTPUT
    p = renderer
    expect(p.render(input))
      .to be_equivalent_to output
  ...
  it "renders external standard, IETF" do
    ...
    INPUT
    output = <<~OUTPUT
      <formattedref><smallcap>S. Bradner</smallcap>. <em><span class='stddocTitle'>Intellectual Property Rights in IETF Technology</span></em>. RFC Series. Available at: <span class='biburl'><fmt-link target='https://www.rfc-editor.org/info/rfc3979'>https://www.rfc-editor.org/info/rfc3979</fmt-link></span>.</formattedref>
    OUTPUT
    p = renderer
    expect(p.render(input))
      .to be_equivalent_to output
  ...
  it "renders external standard, W3C" do
    ...
    INPUT
    output = <<~OUTPUT
      <formattedref><em><span class='stddocTitle'>Time Ontology in OWL</span></em>. Recommendation. Available at: <span class='biburl'><fmt-link target='https://www.w3.org/TR/owl-time/'>https://www.w3.org/TR/owl-time/</fmt-link></span>.</formattedref>
    OUTPUT
    p = renderer
    expect(p.render(input))
      .to be_equivalent_to output
  ...
  it "renders month-year dates" do
    ...
    INPUT
    output = <<~OUTPUT
      <formattedref><smallcap>Jenkins, &#x26; Ruostekoski J.</smallcap>. <em>Controlled manipulation of light by cooperativeresponse of atoms in an optical lattice</em>. Preprint. 2020. Available at: <span class='biburl'><fmt-link target='https://eprints.soton.ac.uk/338797/'>https://eprints.soton.ac.uk/338797/</fmt-link></span>. [viewed: June 2020].</formattedref>
    OUTPUT
    p = renderer
    expect(p.render(input))
      .to be_equivalent_to output
